package com.moduDrive.storage.adapter.in.web.filter;
...
/**
 * Gate on {@code /internal/**} — service-to-service only, and {@code PurgeStoredFileController}
 * takes the acting user's id as a plain request parameter, so whoever reaches it can purge any
 * file's blocks outright (issue #332, file-service's counterpart is #314). Only the gateway not
 ...
 * {@code FILE_NOT_FOUND_IN_STORAGE} body the routes themselves produce — so the response never
 * confirms that an internal route (or the file it names) exists at all.
 *
...
class InternalTokenFilter extends OncePerRequestFilter {
    ...
    private void respondNotFound(HttpServletResponse response) throws IOException {
        response.setStatus(StorageExceptionCase.FILE_NOT_FOUND_IN_STORAGE.getHttpStatus().value());
        response.setContentType(MediaType.APPLICATION_JSON_VALUE);
        response.setCharacterEncoding(StandardCharsets.UTF_8.name());
        objectMapper.writeValue(response.getWriter(), ApiResponse.error(StorageExceptionCase.FILE_NOT_FOUND_IN_STORAGE));
    }
}
