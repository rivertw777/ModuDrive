...
public interface ListFileSharesUseCase {
     ...
     *       the directory they come from. One row per (ancestor, grantee) pair, never collapsed —
     *       includes a grantee who also has a row in {@code shares} directly (see
     *       ListFileSharesService for why: a full revoke needs every one of those rows visible).
     *       FileAccessGuard.resolveRole ignores these entirely once a direct grant exists, but
     *       this view still needs to show them for the owner to actually revoke them.</li>
